...
export class TenantsService {
  ...
  async create(createTenantDto: CreateTenantDto, ownerId: string) {
    ...
    try {
      const userByPhone = await this.userService.findOneUserByUserProperty(
        'phone',
        phone,
      );
      if (userByPhone) {
    ...
    const newUser = await this.userService.findOneUserByUserProperty(
      'email',
      email,
    );
    return newUser;
  ...
  async findAll(ownerId: string) {
    const tenants = await this.userService.findAllByRoleAndOwner(
      Role.TENANT,
      ownerId,
    );
  ...
  async remove(id: string) {
    const flatsWithTenant = await this.flatModel.find({ tenant: id }).exec();

    for (const flat of flatsWithTenant) {
      await this.cleanupUnpaidRentsForFlat(flat._id as string);
    }
  ...
  private async cleanupUnpaidRentsForFlat(flatId: string): Promise<void> {
    try {
      const result = await this.rentModel.deleteMany({
        flat: flatId,
        status: {
          $in: [RentStatus.PENDING, RentStatus.PARTIAL, RentStatus.OVERDUE],
        },
        dueAmount: { $gt: 0 },
      });
      ...
      this.logger.error(`Error cleaning up rents for flat ${flatId}:`, error);
    }
  }
}
